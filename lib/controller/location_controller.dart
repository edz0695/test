...
import 'package:payapp/data/model/response/prediction_model.dart';
...
import 'package:payapp/view/screens/location/widget/module_dialog.dart';
import 'package:payapp/view/screens/location/widget/permission_dialog.dart';
...
class LocationController extends GetxController implements GetxService {
  ...
  void saveAddressAndNavigate(
      AddressModel? address, String? route, bool canRoute, bool isDesktop) {
    if (Get.find<CartController>().cartList.isNotEmpty) {
      Get.dialog(ConfirmationDialog(
        ...
        onYesPressed: () {
          Get.back();
          _setZoneData(address!, route, canRoute, isDesktop);
        },
        onNoPressed: () {
          Get.back();
          Get.back();
  ...
  void _setZoneData(
    ...
    Get.find<LocationController>()
      ...
      if (response.isSuccess) {
        ...
        address.areaIds = [];
        address.areaIds!.addAll(response.areaIds);
        autoNavigate(address, route, canRoute, isDesktop);
      } else {
        Get.back();
  ...
  void autoNavigate(AddressModel? address, String? route, bool canRoute,
    ...
    if (isDesktop && Get.find<SplashController>().configModel!.module == null) {
      ...
      Get.dialog(ModuleDialog(callback: () {
        saveData(address, route, canRoute, isDesktop);
      }),
  ...
  void saveData(AddressModel address, String? route, bool canRoute,
      bool isDesktop) async {
  ...
  Future<String> getAddressFromGeocode(LatLng latLng) async {
    ...
    } else if (response.statusCode == 200 && response.body['status'] != 'OK') {
  ...
  Future<bool> checkLocationActive() async {
    ...
    if (isActiveLocation) {
      ...
      double distance = Geolocator.distanceBetween(
            double.parse(getUserAddress()?.latitude ?? '34834'),
            double.parse(getUserAddress()?.longitude! ?? '329834'),
            myPosition.latitude,
