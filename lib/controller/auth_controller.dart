...
import 'package:payapp/data/model/response/delivery_man_vehicles.dart';
import 'package:payapp/data/model/response/module_model.dart';
import 'package:payapp/data/model/response/response_model.dart';
import 'package:payapp/data/model/response/zone_model.dart';
...
class AuthController extends GetxController implements GetxService {
  ...
  Future<ResponseModel?> registration(SignUpBody signUpBody) async {
    ...
    try {
      ...
      printThis("Response Status: ${response.statusCode}");
      printThis("Response Body: ${response.body}");
      ...
      if (response.statusCode == 200) {
        ...
        await loginSignUpProvider.signUp(
          signUpBody.fName!,
          signUpBody.lName!,
          signUpBody.email!,
          signUpBody.phone!,
        ...
        toast(message: "Registration Successful", bgColor: Colors.green);
        ...
        if (responseJson.containsKey("errors") &&
            responseJson["errors"] is List) {
          final errorMessages = (responseJson["errors"] as List)
              .map((error) => error["message"] as String)
              .join("\n");

          print("Registration Failed: $errorMessages");
          toast(message: errorMessages);
          registerLoading.value = false;
          update();
          return ResponseModel(false, errorMessages);
        } else {
  ...
  Future<ResponseModel> login(String? email, String password) async {
    ...
    if (response.statusCode == 200) {
      if (Get.find<SplashController>().configModel!.customerVerification! &&
        ...
        log("Phone verification ${response.body}");
      } else {
